...
pub const Duration = struct {
    ...
    pub fn formatHuman(this: @This(), alloc: std.mem.Allocator) ![]const u8 {
        const dur_ms: f64 = @floatFromInt(this.ms);

        const st = std.time;
        const ms_per_year = st.ms_per_day * (365.25 - 0.0075);
        const ms_per_month = ms_per_year / 12;
        const tiers = .{
            // .threshold // .divided_by // .unit_text
            .{ st.ms_per_min, st.ms_per_s, "s" },
            .{ st.ms_per_hour, st.ms_per_min, "m" },
            .{ st.ms_per_day, st.ms_per_hour, "h" },
            .{ st.ms_per_week, st.ms_per_day, " day" },
            .{ ms_per_month, st.ms_per_week, " week" },
            .{ ms_per_year, ms_per_month, " month" },
            .{ null, ms_per_year, " year" },
        };

        inline for (tiers) |tier| {
            const always = @TypeOf(tier[0]) == @TypeOf(null);
            if (always or dur_ms < tier[0]) {
                const amount: u64 = @intFromFloat(dur_ms / tier[1]);
                const unit_text_suffix = if (tier[2].len > 1 and amount > 1) "s" else "";
                const fmt = "{}" ++ tier[2] ++ "{s} ago";
                return std.fmt.allocPrint(alloc, fmt, .{ amount, unit_text_suffix });
            }
        }
